...
#define FUNC( N, val )     ( PORT_FUNC_MODE_##val << __concat_3( PORT_FUNC_MODE, N, _Pos ))
#define OR_FUNC( N, val )  | FUNC( N, val )

#define DIGIT( N )         ( 1UL << N )
#define OR_DIGIT( N )      | DIGIT( N )

#define PWR( N, val )      ( PORT_PWR_##val << __concat_3( PORT_PWR, N, _Pos ))
#define OR_PWR( N, val )   | PWR( N, val )
...
void SystemInit1( void )
{
	...
	/* Switch CPU PLL to HSE oscillator 16 MHz */
	...
	/* Initialize CPU PLL, 16 MHz * (4 + 1) = 80 MHz */
